...
def main():
    ...
    for detector in os.scandir('./DeepfakeBench/training/config/detector/'):
        if detector.name in skip_please:
            print(f"skipping {detector.name}")
            continue
        ...
        with open(detector.path, 'r') as f:
            config = yaml.safe_load(f)
        ...
        if weights_path:
            ...
            if detector.name in ['i3d.yaml', 'ftcn.yaml']:
                model.resnet.load_state_dict(ckpt, strict=True)
            else:
